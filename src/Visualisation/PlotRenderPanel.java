...
public class PlotRenderPanel extends JPanel {
    private JungleStepMap map;
    private MapSimulation simulation;
    private int totalDays;
    ArrayList<Integer> animalsPopulation;
    ArrayList<Integer> grassPopulation;

    public PlotRenderPanel(JungleStepMap map, MapSimulation simulation) {
        this.animalsPopulation = new ArrayList<>();
        this.grassPopulation = new ArrayList<>();
        this.map = map;
    ...
    protected void paintComponent(Graphics g) {
        ...
        this.setSize((int) (simulation.frame.getWidth() * 0.4), this.simulation.frame.getHeight() - 38); //38 is toolbar size
        this.setLocation(0, 0);
        int width = this.getWidth();
        int height = this.getHeight();
        this.totalDays++;
        g.drawString("Total days: " + this.totalDays, 10, height - 20);

        this.animalsPopulation.add(this.map.animalsList.size());
        this.grassPopulation.add(this.map.grassList.size());
        ...
        int i = 0;
        for (int j : this.animalsPopulation) {
